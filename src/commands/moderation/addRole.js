...
module.exports = {
    ...
    options: [
        {
            name: "user",
            description: "The user you want to add a role",
            required: true,
    ...
    callback: async (client, interaction) => {
        ...
        if (addRolePosition >= requestUserRolePosition && interaction.member.id != interaction.guild.ownerId) {
            await interaction.editReply("You can't add a higher role or the same role as your highest role.");
            return;
        }

        if (addRolePosition >= botRolePostion) {
            await interaction.editReply("I can't add a higher role or the same role as my highest role.");
            return;
        }

        try {
            await targetUser.roles.add(targetRole);
            await interaction.editReply(`${targetRole} role has been successfully added to ${targetUser}`);
        } catch (error) {
            console.log(`There was an error when adding a role: ${error}.`);
            await interaction.editReply(`Can't add the role`);
        }
    },
};
